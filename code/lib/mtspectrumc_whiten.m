...
function [S,S1,f,Serr]=mtspectrumc_whiten(data,params)
...
J=J(findx,:,:);
S=squeeze(mean(conj(J).*J,2));%power

%=============Whitening(add)=================
%Normlize power
   S1 = S.*repmat(f',1,size(S,2)); % whitening

if trialave; S=squeeze(mean(S,2));end;
if nargout==4; % number of function output arguments
% if nargout==3;
