%modify by Qun Li 2017
%S1 is Whitening correction

function [S,S1,t,f]= mtspecgramc_whiten(data,movingwin,params)
%function [S,S1,t,f]=mtspecgramc(data,movingwin,params)
...
% data=change_row_to_column(data); %no need to do

%data=10^3*data;%-----mv %no need to gain 
[N,Ch]=size(data);
...
if trialave
    [S,S1] = deal(zeros(nw,Nf));
    if nargout==5; Serr=zeros(2,nw,Nf); end;%modify
    %if nargout==4; Serr=zeros(2,nw,Nf); end;
else
    [S,S1] = deal(zeros(nw,Nf,Ch));
    if nargout==5; Serr=zeros(2,nw,Nf,Ch); end;%modify
...
for n=1:nw;
   ...
   %if nargout==4
     ...
     [s,s1,f,~]=mtspectrumc_whiten(datawin,params);%modify
   end
   S(n,:,:) = s;
   S1(n,:,:) = s1;
end;
...
if nargout==5;Serr=squeeze(Serr);end;%modify
